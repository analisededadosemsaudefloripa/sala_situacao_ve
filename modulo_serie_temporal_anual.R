...
#'
#'Diferente dos outros módulos de série temporal, este trabalha com dados anuais
#'
#######################################################################
##Pacotes
#######################################################################
...
serie_temporal_anual_UI <- function(id,banco){
        ns <- NS(id)
                tagList(
                fluidPage(
                   sidebarLayout(
...
serie_temporal_anual <- function(input, output, session, banco){
        
banco_preparado <- reactive({
        require(input$indicador_banco)
        subset(banco, banco$INDICADOR == input$indicador_banco)
        
})

inicio <- reactive({
        a <- banco_preparado()$ANO
...
banco_pre <- reactive({         
        ...
        banco_pre <- ts(banco_pre[,-c(1,2)],start = inicio(), frequency = 1, deltat = 1)
        banco_pre 
})        
